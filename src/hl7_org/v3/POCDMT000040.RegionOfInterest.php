...
class POCDMT000040.RegionOfInterest {
	...
	/**
	 * @XmlElement	(type="\hl7_org\v3\CS", name="code")
	 */
	protected $code;

	/**
	 * @XmlElement	(type="\hl7_org\v3\POCDMT000040.RegionOfInterest.value", collection="true", name="value")
	 */
	protected $value;
	...
	protected function validateCode($code) {
		if ( ! $code instanceof \hl7_org\v3\CS ) {
	...
	protected function validateValue($value) {
		if ( ! $value instanceof \hl7_org\v3\POCDMT000040.RegionOfInterest.value ) {
			$value = new \hl7_org\v3\POCDMT000040.RegionOfInterest.value ($value);
		}
		$count = count($value);
		if ($count < 1) {
		...
		foreach ($value as $entry) {
			if (!($entry instanceof POCDMT000040.RegionOfInterest.value)) {
				throw new \Exception(sprintf('Supplied %s value was not %s', 'value', 'POCD_MT000040.RegionOfInterest.value'));
			}
		}
