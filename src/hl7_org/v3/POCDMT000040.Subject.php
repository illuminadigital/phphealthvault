 ...
 * })
 * @XmlEntity	(xml="POCD_MT000040.Subject")
 */
class POCDMT000040.Subject {
	/**
	 */
	...
	/**
	 * @XmlElement	(type="\hl7_org\v3\POCDMT000040.RelatedSubject", name="relatedSubject")
	 */
	protected $relatedSubject;
	...
	public function __construct($realmCode = NULL, $typeId = NULL, $templateId = NULL, $awarenessCode = NULL, $relatedSubject = NULL, $nullFlavor = NULL, $typeCode = NULL, $contextControlCode = NULL) {
		...
		$this->relatedSubject = ($relatedSubject===NULL) ? NULL : $this->validateRelatedSubject($relatedSubject);
	...
	public function getRelatedSubject() {
		if ($this->relatedSubject===NULL) {
			$this->relatedSubject = $this->createRelatedSubject();
		}
		return $this->relatedSubject;
	}
	
	protected function createRelatedSubject() {
		return new \hl7_org\v3\POCDMT000040.RelatedSubject();
	}

	public function setRelatedSubject($relatedSubject) {
		$this->relatedSubject = $this->validateRelatedSubject($relatedSubject);
	}

	protected function validateRelatedSubject($relatedSubject) {
		if ( ! $relatedSubject instanceof \hl7_org\v3\POCDMT000040.RelatedSubject ) {
			$relatedSubject = new \hl7_org\v3\POCDMT000040.RelatedSubject ($relatedSubject);
		}
	...
	protected function createTypeCode() {
		return new \hl7_org\v3\ParticipationTargetSubject();
	}
	...
	protected function validateTypeCode($typeCode) {
		if ( ! $typeCode instanceof \hl7_org\v3\ParticipationTargetSubject  && ! is_null($typeCode) ) {
			$typeCode = new \hl7_org\v3\ParticipationTargetSubject ($typeCode);
		}
...
} // end class POCDMT000040.Subject
