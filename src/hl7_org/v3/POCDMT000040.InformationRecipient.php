 ...
 * })
 * @XmlEntity	(xml="POCD_MT000040.InformationRecipient")
 */
class POCDMT000040.InformationRecipient {
	/**
	 */
	...
	/**
	 * @XmlElement	(type="\hl7_org\v3\POCDMT000040.IntendedRecipient", name="intendedRecipient")
	 */
	protected $intendedRecipient;
	...
	public function __construct($realmCode = NULL, $typeId = NULL, $templateId = NULL, $intendedRecipient = NULL, $nullFlavor = NULL, $typeCode = NULL) {
		$this->realmCode = ($realmCode===NULL) ? NULL : $this->validateRealmCode($realmCode);
		$this->typeId = ($typeId===NULL) ? NULL : $this->validateTypeId($typeId);
		$this->templateId = ($templateId===NULL) ? NULL : $this->validateTemplateId($templateId);
		$this->intendedRecipient = ($intendedRecipient===NULL) ? NULL : $this->validateIntendedRecipient($intendedRecipient);
	...
	public function getIntendedRecipient() {
		if ($this->intendedRecipient===NULL) {
			$this->intendedRecipient = $this->createIntendedRecipient();
		}
		return $this->intendedRecipient;
	}
	
	protected function createIntendedRecipient() {
		return new \hl7_org\v3\POCDMT000040.IntendedRecipient();
	}

	public function setIntendedRecipient($intendedRecipient) {
		$this->intendedRecipient = $this->validateIntendedRecipient($intendedRecipient);
	}

	protected function validateIntendedRecipient($intendedRecipient) {
		if ( ! $intendedRecipient instanceof \hl7_org\v3\POCDMT000040.IntendedRecipient ) {
			$intendedRecipient = new \hl7_org\v3\POCDMT000040.IntendedRecipient ($intendedRecipient);
		}
	...
	protected function createTypeCode() {
		return new \hl7_org\v3\XInformationRecipient();
	}
	...
	protected function validateTypeCode($typeCode) {
		if ( ! $typeCode instanceof \hl7_org\v3\XInformationRecipient  && ! is_null($typeCode) ) {
			$typeCode = new \hl7_org\v3\XInformationRecipient ($typeCode);
		}
	
		return $typeCode;
	}
} // end class POCDMT000040.InformationRecipient
