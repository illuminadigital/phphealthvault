 ...
 * })
 * @XmlEntity	(xml="POCD_MT000040.DocumentationOf")
 */
class POCDMT000040.DocumentationOf {
	/**
	 */
	...
	/**
	 * @XmlElement	(type="\hl7_org\v3\POCDMT000040.ServiceEvent", name="serviceEvent")
	 */
	...
	public function __construct($realmCode = NULL, $typeId = NULL, $templateId = NULL, $serviceEvent = NULL, $nullFlavor = NULL, $typeCode = NULL) {
		$this->realmCode = ($realmCode===NULL) ? NULL : $this->validateRealmCode($realmCode);
		$this->typeId = ($typeId===NULL) ? NULL : $this->validateTypeId($typeId);
		$this->templateId = ($templateId===NULL) ? NULL : $this->validateTemplateId($templateId);
		$this->serviceEvent = ($serviceEvent===NULL) ? NULL : $this->validateServiceEvent($serviceEvent);
	...
	public function getServiceEvent() {
		if ($this->serviceEvent===NULL) {
			$this->serviceEvent = $this->createServiceEvent();
		}
		return $this->serviceEvent;
	}
	
	protected function createServiceEvent() {
		return new \hl7_org\v3\POCDMT000040.ServiceEvent();
	}

	public function setServiceEvent($serviceEvent) {
		$this->serviceEvent = $this->validateServiceEvent($serviceEvent);
	}

	protected function validateServiceEvent($serviceEvent) {
		if ( ! $serviceEvent instanceof \hl7_org\v3\POCDMT000040.ServiceEvent ) {
			$serviceEvent = new \hl7_org\v3\POCDMT000040.ServiceEvent ($serviceEvent);
		}
...
} // end class POCDMT000040.DocumentationOf
