 ...
 * })
 * @XmlEntity	(xml="POCD_MT000040.Material")
 */
class POCDMT000040.Material {
	/**
	 */
	...
	/**
	 * @XmlElement	(type="\hl7_org\v3\ST", name="lotNumberText")
	 */
	protected $lotNumberText;
	...
	public function __construct($realmCode = NULL, $typeId = NULL, $templateId = NULL, $code = NULL, $name = NULL, $lotNumberText = NULL, $nullFlavor = NULL, $classCode = NULL, $determinerCode = NULL) {
		$this->realmCode = ($realmCode===NULL) ? NULL : $this->validateRealmCode($realmCode);
		$this->typeId = ($typeId===NULL) ? NULL : $this->validateTypeId($typeId);
		$this->templateId = ($templateId===NULL) ? NULL : $this->validateTemplateId($templateId);
		$this->code = ($code===NULL) ? NULL : $this->validateCode($code);
		$this->name = ($name===NULL) ? NULL : $this->validateName($name);
		$this->lotNumberText = ($lotNumberText===NULL) ? NULL : $this->validateLotNumberText($lotNumberText);
	...
	public function getLotNumberText() {
		if ($this->lotNumberText===NULL) {
			$this->lotNumberText = $this->createLotNumberText();
		}
		return $this->lotNumberText;
	}
	
	protected function createLotNumberText() {
		return new \hl7_org\v3\ST();
	}

	public function setLotNumberText($lotNumberText) {
		$this->lotNumberText = $this->validateLotNumberText($lotNumberText);
	}

	protected function validateLotNumberText($lotNumberText) {
		if ( ! $lotNumberText instanceof \hl7_org\v3\ST  && ! is_null($lotNumberText) ) {
			$lotNumberText = new \hl7_org\v3\ST ($lotNumberText);
		}
...
} // end class POCDMT000040.Material
