 ...
 * })
 * @XmlEntity	(xml="POCD_MT000040.Location")
 */
class POCDMT000040.Location {
	/**
	 */
	...
	/**
	 * @XmlElement	(type="\hl7_org\v3\POCDMT000040.HealthCareFacility", name="healthCareFacility")
	 */
	protected $healthCareFacility;
	...
	public function __construct($realmCode = NULL, $typeId = NULL, $templateId = NULL, $healthCareFacility = NULL, $nullFlavor = NULL, $typeCode = NULL) {
		$this->realmCode = ($realmCode===NULL) ? NULL : $this->validateRealmCode($realmCode);
		$this->typeId = ($typeId===NULL) ? NULL : $this->validateTypeId($typeId);
		$this->templateId = ($templateId===NULL) ? NULL : $this->validateTemplateId($templateId);
		$this->healthCareFacility = ($healthCareFacility===NULL) ? NULL : $this->validateHealthCareFacility($healthCareFacility);
	...
	public function getHealthCareFacility() {
		if ($this->healthCareFacility===NULL) {
			$this->healthCareFacility = $this->createHealthCareFacility();
		}
		return $this->healthCareFacility;
	}
	
	protected function createHealthCareFacility() {
		return new \hl7_org\v3\POCDMT000040.HealthCareFacility();
	}

	public function setHealthCareFacility($healthCareFacility) {
		$this->healthCareFacility = $this->validateHealthCareFacility($healthCareFacility);
	}

	protected function validateHealthCareFacility($healthCareFacility) {
		if ( ! $healthCareFacility instanceof \hl7_org\v3\POCDMT000040.HealthCareFacility ) {
			$healthCareFacility = new \hl7_org\v3\POCDMT000040.HealthCareFacility ($healthCareFacility);
		}
	...
	protected function createTypeCode() {
		return new \hl7_org\v3\ParticipationTargetLocation();
	}
	...
	protected function validateTypeCode($typeCode) {
		if ( ! $typeCode instanceof \hl7_org\v3\ParticipationTargetLocation  && ! is_null($typeCode) ) {
			$typeCode = new \hl7_org\v3\ParticipationTargetLocation ($typeCode);
		}
	
		return $typeCode;
	}
} // end class POCDMT000040.Location
