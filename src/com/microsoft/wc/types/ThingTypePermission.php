...
class ThingTypePermission {
	...
	/**
	 * @XmlElement	(type="\com\microsoft\wc\types\Permissions", name="online-access-permissions")
	 */
	protected $onlineAccessPermissions;

	/**
	 * @XmlElement	(type="\com\microsoft\wc\types\Permissions", name="offline-access-permissions")
	 */
	...
	protected function createOnlineAccessPermissions() {
		return new \com\microsoft\wc\types\Permissions();
	}
	...
	protected function validateOnlineAccessPermissions($onlineAccessPermissions) {
		...
		if ( ! $onlineAccessPermissions instanceof \com\microsoft\wc\types\Permissions  && ! is_null($onlineAccessPermissions) ) {
			$onlineAccessPermissions = new \com\microsoft\wc\types\Permissions ($onlineAccessPermissions);
		}
	...
	protected function validateOfflineAccessPermissions($offlineAccessPermissions) {
		...
		if ( ! $offlineAccessPermissions instanceof \com\microsoft\wc\types\Permissions  && ! is_null($offlineAccessPermissions) ) {
			$offlineAccessPermissions = new \com\microsoft\wc\types\Permissions ($offlineAccessPermissions);
		}
